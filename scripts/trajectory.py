...
def extract_ptcld(vl_path, vr_path):
    ...
    ptcld = return_ptcld(vl_path, vr_path, side_range, fwd_range)
...
def create_map(vr_time, vl_time):
    ...
    x_gt, y_gt = [pose[0, 3] for pose in gt_poses], [pose[1, 3] for pose in gt_poses]
    ...
    for idx in range(200):
        ...
        ptcld = extract_ptcld(vl_im_path, vr_im_path)
        scan = ptcld[0:3, :] + np.array([[x_gt[idx]], [y_gt[idx]], [0.0]])
        if map is None:
            map = scan
        else:
...
if __name__ == '__main__':
    # timestamps = [int(file[0:-4]) for file in sorted(os.listdir('../data/oxford/sample/velodyne_left')) if file.endswith('.png')]
